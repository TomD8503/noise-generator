...
class Project:
    ...
    best_phases_rad: np.ndarray = None
   ...
   # def _generate_phase_lut(self):
    ...
    # PUBLIC METHODS
    # ============================================================
    def generate_phases_rad(self):
        #self._ensure_initialized()
        return np.random.uniform(-np.pi, np.pi, size=len(self._freqs))

    def generate_waveform_phases_rad(self, phases_rad):
        self._ensure_initialized()
        # Synthesize waveform
        spectrum = np.zeros(len(self._freqs), dtype=np.complex64)
        spectrum[self._active_idx] = (
            self._magnitude_mask[self._active_idx] *
            np.exp(1j * phases_rad[self._active_idx])
        )
    ...
    def generate_and_evaluate(self):
        phases = self.generate_phases_rad()
        waveform = self.generate_waveform_phases_rad(phases)
        cf_db = self.crest_factor(waveform)
        error = abs(cf_db - self.target_crest_db)
        return waveform, phases, error, cf_db

    def is_new_best(self, phases_rad, error, crest_db):
        if error < self.best_error:
            self.best_error = error
            self.best_crest_db = crest_db
            self.best_phases_rad = phases_rad
            self.last_modified = time.time()
            return True
        return False
    ...
    def get_info(self):
        ...
        return {
            ...
            'best': {
                ...
                'has_candidate': self.best_phases_rad is not None
            }
        }
